...
// 可任意增刪及 rename class、function，如有多 class 放在同一檔案即可，並以註解說明修改原因
class OrderManager {
	public $c = null;

	public function run($args, $creditCard, $user) {
		$this->c = new Client([
			'headers' => [
		...
		$order->status = 'paying';
		$order->save();
		$response = $this->c->request('POST', 'https://hellopay.com/pay', [
			'body' => json_encode([
	...
	public function createBookingSession() {
		...
		$response = $this->c->request(
			'POST', 
			'https://helloticket.com/create', 
			['body' => json_encode(['authKey' => '1234567890'])]
		);
		if ($response->getStatusCode() == "200") {
	...
	public function bookFlight($bookingSession) {
		...
		$response = $this->c->request(
			'POST', 
			'https://helloticket.com/book', 
			['body' => json_encode(['id' => $bookingSession['bookingId']])]
		);
		if ($response->getStatusCode() == "200") {
			return true;
		}
	...
	public function queryFlight($origin, $destination, $date) {
		...
		$response = $this->c->request('POST', 'https://helloticket.com/query', [
			'body' => json_encode([
				'origin' => $origin, 
